...
import { BookOpen, Bot, Compass, Flag, LayoutDashboard, LogOut, Menu, Shield, Target, X, User, UsersRound } from 'lucide-react';
...
const Layout = ({ children }) => {
  ...
  const navigation = (
    <>
      <Link to="/dashboard" className="mb-9 flex items-center gap-2 px-3">
        <span className="grid h-9 w-9 place-items-center rounded-xl bg-brand-primary shadow-sm" style={{ color: '#ffffff' }}><Shield size={19} color="#ffffff" /></span>
      ...
      <nav className="space-y-1" aria-label="Primary navigation">
        {navItems.map(({ label, path, icon: Icon }) => {
          ...
          return (
            <Link key={label} to={path} className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold transition-colors ${active ? 'bg-brand-light text-brand-primary' : 'text-brand-muted hover:bg-brand-light hover:text-brand-primary'}`}>
              <Icon size={18} strokeWidth={1.8} />{label}
            </Link>
          );
        })}
        {/* AI Assistant — always white Bot icon on green badge */}
        {(() => {
          const active = location.pathname === '/assistant';
          return (
            <Link to="/assistant" className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold transition-colors ${active ? 'bg-brand-light text-brand-primary' : 'text-brand-muted hover:bg-brand-light hover:text-brand-primary'}`}>
              <span style={{ display: 'grid', height: '20px', width: '20px', placeItems: 'center', borderRadius: '6px', backgroundColor: '#064E3B', color: '#ffffff', flexShrink: 0 }}>
                <Bot size={13} strokeWidth={2} color="#ffffff" />
              </span>
              AI Assistant
            </Link>
          );
  ...
  return <div className="min-h-screen bg-brand-bg">
    ...
    <main className="lg:pl-64 flex flex-col min-h-screen">
      <AppHeader />
      <div className={`flex-1 ${location.pathname === '/dashboard' || location.pathname === '/plan' ? '' : 'legacy-page'}`}>{children}</div>
      <AppFooter />
    </main>
  </div>;
};
