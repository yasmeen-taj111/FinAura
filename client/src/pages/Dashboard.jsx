...
const Dashboard = () => {
  ...
  const firstName = profile?.userId?.name?.split(' ')[0] || 'there';
  ...
  return <div className="page-shell px-5 py-8 md:px-8 lg:px-12 lg:py-10">
    <div className="mx-auto max-w-6xl">
      <header className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="eyebrow">Your financial journey</p><h1 className="mt-2 font-serif text-4xl text-brand-ink md:text-5xl">Good morning, {firstName}.</h1><p className="mt-3 text-sm leading-6 text-brand-muted">Small, informed decisions add up. Here is your clearest next step.</p></div><Link to="/plan" className="inline-flex items-center gap-2 text-sm font-semibold text-brand-primary hover:underline">View your plan <ArrowRight size={16} /></Link></header>
